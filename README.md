# homebridge-HTTP-CO2-sensor

This [Homebridge](https://github.com/nfarina/homebridge) plugin can be used integrate your Co2 sensor which has a 
...
instructions.  
Then run the following command to install `homebridge-http-co2-sensor`
...
When using the 'push' concept, the http device itself sends the updated value to `homebridge-http-co2-sensor` 
whenever values change. This is more efficient as the new value is updated instantly and 
`homebridge-http-co2-sensor` does not need to make needless requests when the value didn't actually change.  
However because the http device needs to actively notify the `homebridge-http-co2-sensor` there is more 
...
`homebridge-HTTP-CO2-sensor` can be used together with 
...
Down here is an example on how to configure `homebridge-HTTP-CO2-sensor` to work with your implementation of the 
